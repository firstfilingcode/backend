                                ...
                                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline">
                                    ...
                                    <tbody class="product_list_show">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($value->status==1)
                                                <button data-toggle="modal" data-target="#Modal_id"
                                                    data-id="{{ $value->id }}" data-name="Active"
                                                    class="btn btn-success btn-sm btn-soft-success waves-effect waves-light sa-params branch_status"
                                                    style="display:inline">Active</button>
                                                @else
                                                <button data-toggle="modal" data-target="#Modal_id"
                                                    data-id="{{ $value->id }}" data-name="Inactive"
                                                    class="btn btn-danger btn-sm btn-soft-danger waves-effect waves-light branch_status"
                                                    style="display:inline">Inactive</button>
                                                @endif
                                            </td>
                                            <td class="d-flex">
                                                <a href="{{  route('admin.branch.show',$value->id)  }}"
                                                    class=" text-danger  ml-3" title="Show Blog"><i
                                                        class="fa fa-eye"></i></a>
                                                <a href="{{  route('admin.branch.edit',$value->id)  }}"
                                                    class=" text-success  ml-3" title="Edit Branch"><i
                                                        class="fa fa-edit"></i></a>
                                                <a data-id="{{$value->id}}" data-location="countries"
                                                    class="branch_delete text-danger  ml-3" data-toggle="tooltip"
                                                    data-placement="top" title="Delete"
                                                    onclick="document.getElementById('id01').style.display='block'"><i
                                                        class="fa fa-trash-o"></i></a> 
                                                {!! Form::open(['method' => 'DELETE','route' => ['admin.branch.destroy',
                                                $value->id]]) !!}
                                                {!! Form::close() !!}
                                            </td>
                                        </tr>
        ...
        <div class="modal-content bg_color">
            ...
            <form action="{{ route('admin.branch.destroy')}}" method="post">
                ...
                <div class="modal-body">
                    <input type="hidden" id="branch_delete_id" name="branch_delete_id" />
...
<script>
    ...
    $('.branch_delete').click(function(){
      ...
      $("#branch_delete_id").val($(this).data('id'))
...
<div class="modal" id="Modal_id">
    <div class="modal-dialog">
        <div class="modal-content bg_color">
            ...
            <!-- Modal body -->
            <form action="{{ route('admin.branch.status') }}" method="post">
                @csrf
                <div class="modal-body">
