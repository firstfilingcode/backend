                            ...
                            <div class="col-12">
                                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline">
                                  ...
                                  @foreach($data as $key => $value)
                                        <tr>
                                            <td><a href="{{  route('admin.rm.show',$value->id)  }}">{{$value['name'] ?? ''}}</a></td>
                                            <td>{{$value['mobile'] ?? ''}}</td>
                                            <td>{{$value['email'] ?? ''}}</td>
                                        ...
                                        <button  data-id="{{ $value->id }}" data-name="{{$value->status==0 ? '1' : '0'}}" class="{{$value->status==0 ? 'btn btn-success' : 'btn btn-danger'}} btn-sm btn-soft-success waves-effect waves-light sa-params user_status" style ="display:inline">{{$value->status==0 ? 'Acitve' : 'Inactive'}}</button>
                                            ...
                                            <td>
                                                <a href="{{  route('admin.rm.show',$value->id)  }}" class=" text-success btn-xs ml-3" title="View Account"><i
                                                        class="fa fa-eye"></i></a>
                                                <a href="{{  route('admin.rm.edit',$value->id)  }}" class=" text-success btn-xs ml-3" title="Edit Account"><i
 ...
 <div class="modal" id="Modal_id1">
    <div class="modal-dialog">
        <div class="modal-content bg_color">
            ...
            <!-- Modal body -->
            <form action="{{ route('admin.rm.destroy')}}" method="post">
                @csrf
...
<div class="modal" id="Modal_id">
    <div class="modal-dialog">
        <div class="modal-content bg_color">
            ...
            <!-- Modal body -->
            <form action="{{ route('admin.rm.status') }}" method="post">
                @csrf
                <div class="modal-body">
