@extends('admin.layouts.app')

@section('title') @lang('translation.Dashboard')
@endsection @section('content')

<div class="content-wrapper">
    <section class="content pt-3">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12 col-md-12">
                    <div class="card card-outline card-orange">
                        <div class="card-header bg-primary">
                            <h3 class="card-title">
                                <i class="fa fa-address-book-o"></i> &nbsp; {{ __('Page Route') }}
                            </h3>
                           <div class="card-tools">
                                <!-- <a href="{{url('admin/order_status/create')}}" class="btn btn-warning text-white btn-sm"><i
                                        class="fa fa-plus"></i>{{ __('Add') }}</a>-->
                                        
                                <a href="{{ URL::previous() }}" class="btn btn-warning text-white btn-sm"><i
                                        class="fa fa-arrow-left"></i>{{ __('Back') }}</a>
                            </div>
                        </div>

                        <div class="row m-2">
                            <div class="col-12">
                                <table id="example1" class="table table-bordered table-striped dataTable dtr-inline">
                                    <thead>
                                        <tr role="row">
                                            <th>Sr.no</th>
                                            <th>Route Name</th>
                                            <th>Name</th>
                                            <th>Service Type</th>
                                            <th>Service Sub Type</th>
                                            <th>Order BY</th>
                                            <th>Action</th>
                                        </tr>
                                    </thead>

                                    <tbody class="product_list_show">
                                          @if(!empty($data))
                                            @php
                                                $i=1;
                                               
                                            @endphp
                                  @foreach($data as $key => $value)
                                        <tr>
                                            <td>{{$i++}}</td>
                                            <td>{{$value['route'] ?? ''}}</td>
                                          
                                            <td>{{$value['page_name'] ?? ''}}</td>
                                            <td>{{$value['service_types'] ?? ''}}</td>
                                            <td>{{$value['service_sub_type'] ?? ''}}</td>
                                            <td>{{$value['order_By'] ?? ''}}</td>
                                            
                                            
                                           
                                            <td class="d-flex">
                                                
                                                <a href="{{  route('admin.routes.edit',$value->id)  }}" class=" text-success btn-xs ml-3" title="Edit Page Route"><i
                                                        class="fa fa-edit"></i></a>
                                                 <a data-id="{{$value->id}}" data-location="countries"
                                                    class="expense_delete text-danger  ml-3" data-toggle="tooltip"
                                                    data-placement="top" title="Delete"
                                                    onclick="document.getElementById('id01').style.display='block'"><i
                                                        class="fa fa-trash-o"></i></a> 
                                            </td>
                                           
                                        </tr>
                                         @endforeach
                            
                            @endif
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>



<script>
    $('#close,#close1').click(function () {
        $('#id01').hide()
    })

    $('.expense_delete').click(function(){
        
      $("#expense_delete_id").val($(this).data('id'))
      $("#Modal_id1").modal("show");
      
   })
</script>
<div class="modal" id="Modal_id1">
    <div class="modal-dialog">
        <div class="modal-content bg_color">
            <!-- Modal Header -->
            <div class="modal-header">
                <h4 class="modal-title text-white">{{ __('Delete Data On Database') }}</h4>
                <button type="button" class="btn-close" data-dismiss="modal">
                    <i class="fa fa-times" aria-hidden="true"></i>
                </button>
            </div>
            <!-- Modal body -->
            <form action="{{ route('admin.routes.destroy')}}" method="post">
                @csrf
                <div class="modal-body">
                    <input type="hidden" id="expense_delete_id" name="routes_delete_id" />
                    
                    <h5 class="text-white">
                        {{ __(' Are you sure you want to delete this data......') }}?
                    </h5>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default waves-effect remove-data-from-delete-form"
                        data-dismiss="modal">
                        {{ __('Close') }}
                    </button>
                    <button type="submit" class="btn btn-danger waves-effect waves-light">
                        {{ __('yes') }}
                    </button>
                </div>
            </form>
        </div>
    </div>
</div>
<script>
    $('#close,#close1').click(function () {
        $('#id01').hide()
    })

    $('.user_delete').click(function(){

      $("#order_delete_id").val($(this).data('id'))
      $("#Modal_id1").modal("show");
      
   })
</script>
<style>
    .btn-xs {
  padding: .125rem .25rem;
  font-size: 17px;
  line-height: 1.5;
  border-radius: .15rem;
}
</style>
@endsection